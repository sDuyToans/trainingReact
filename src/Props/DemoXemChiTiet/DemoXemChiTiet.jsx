...
export default class DemoXemChiTiet extends Component {
  ...
  renderProduct = () => {
    return data.map((product, index) => {
      return (
        <div className="col-4">
          <ProductItem item={product} xemChiTiet={this.xemChiTiet}/>
        </div>
      );
    });
  };
  ...
  xemChiTiet = (productClick) => {
    this.setState({productDetail : productClick})
  };
  render() {
    let { maSP, giaBan, hinhAnh, manHinh, ram, cameraSau, cameraTruoc, rom, heDieuHanh } = this.state.productDetail;
    return (
